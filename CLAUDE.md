...
- [2026-07-20] Homepage rebuilt in Elementor to match https://dsmdesigns.dreamhosters.com/ content (not just layout — reference pulled and diffed directly). Corrected: testimonials (real names — SOW Salon/Bryce Francis/Mane Edit Salon, not invented ones), Why DSM copy (real longer paragraphs, incl. AZ/CA/ID/NM/NV/UT/OR/WA shipping surcharge detail), Financing section (renamed "Brickhouse Capital Financing" with real copy/image). Added 3 sections that existed on reference but not in the inherited placeholder build: General Inquiries (embeds existing CF7 form 105), "Design Your Signature Space" CTA, Project Gallery (6 curated images, imported from reference site into media library). Removed "Shop by category" section — confirmed not present on the real reference homepage (those are mega-menu items only, not a homepage section). Source: Jay ("it is the reference").
- [2026-07-20] **Instagram feed section deferred, not built.** Reference homepage has a live Instagram feed (dsm_designs) requiring "Instagram Feed Pro" — a paid third-party plugin, not in current stack or sow-spec's scope/budget. Needs a licensing decision + the client's Instagram Business API access before it can be built. Flagged, not actioned.
